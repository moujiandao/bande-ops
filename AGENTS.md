...
> Canonical project instructions for all coding agents. `CLAUDE.md` imports this file.
> Edit this file only. Current operational status and go-live gaps live in
> `docs/go-live-readiness.md` and GitHub Issues.
...
- **SVD is denominated in BOXES; FBA and AWD in units.** `lib/reorder/supply.ts` converts SVD boxes to units via the per-SKU `svd_units_per_box` (migration `0015`) before anything is summed - it is the one place the conversion lives, and FBA/AWD are never multiplied. An unset pack size on a SKU with SVD stock blocks as `unknown-svd-units-per-box`; on a SKU with zero SVD stock it is ignored (zero boxes is zero units under any factor). The original report claimed AWD was boxes too; a magnitude check refuted it (500 AWD boxes × 60 ⇒ 30,000 units of one SKU), which is why the factor is SVD-only and named for it.
- **`svd_units_per_box` has NO global-default fallback, unlike lead time / safety stock / coverage.** Those are policy choices where one default sensibly applies to every SKU; a pack size is a physical per-product fact, so a global default would fabricate rather than approximate. It resolves per-SKU or stays UNKNOWN - see the comment in `lib/settings/resolve.ts`. Migration `0016` made lead time and safety stock nullable on per-SKU rows (required on the default row via a check constraint) so a row can carry only a box size and inherit the rest.
...
- Do not call SVD outside `lib/svd`, put SVD or Amazon secrets anywhere they can reach the browser or a log, or add an automatic/unattended SVD refresh (authenticated-user-triggered only).
